package com.review.module.checklist.repository;

import com.review.module.checklist.entity.ComplianceChecklistDO;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface ComplianceChecklistRepository extends JpaRepository<ComplianceChecklistDO, Long> {

    List<ComplianceChecklistDO> findByContentTypeAndEnabledTrue(String contentType);

    List<ComplianceChecklistDO> findByDocumentTypeAndEnabledTrue(String documentType);

    List<ComplianceChecklistDO> findByContractTypeAndEnabledTrue(String contractType);

    List<ComplianceChecklistDO> findByTenantIdAndEnabledTrue(Long tenantId);

    List<ComplianceChecklistDO> findByEnabledTrue();
}
